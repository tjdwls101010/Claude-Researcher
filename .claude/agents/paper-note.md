...
Every figure appears as `![alt text](images/<id>v<n>/<name>.png)` followed by its caption. If the alt text is non-empty it is a faithful description; use it. Whether or not it is, open the PNG with `Read` when the figure carries results or a mechanism (result charts, architecture diagrams, example transcripts), because what a figure shows is substance. Decorative or purely illustrative figures need no visit.
