...
Table 2가 통제된 비교가 아니라는 것은 저자들이 명시하지만, 초록이 내세우는 "최고 공개 메모리 시스템 대비 5.1점"과 "최고 공개 장기 에이전트 대비 37.4점"이라는 수치는 그 단서 없이 읽히기 쉽다. 특히 37.4점은 백본이 다른 MiniMax M3 + ReAct(49.3)와의 차이여서, 컨텍스트 관리 방식의 기여와 백본의 기여가 분리되지 않는다. 백본을 통제한 유일한 비교는 Table 3이고 거기서 Scroll과 CodeAct 에이전트의 차이는 256K에서 86.7 대 85.3, 즉 1.4점이다.

eviction 메커니즘의 효과는 실제로는 얇게 검증됐다. Table 3의 $\Delta$가 CodeAct 에이전트에서 -4.0, Scroll에서 -2.6이고, ablation에서 eviction index 제거는 전체 1.8점만 앗아 간다. 두 메모리 벤치마크는 세션 경계마다 raw 컨텍스트를 비우는 프로토콜로 돌기 때문에 eviction이 실제로 압박받는 조건인지 불분명하다. 즉 논문에서 가장 큰 이득은 손실 없는 Event Log와 영속 kernel(합쳐서 0.20 → 0.73)에서 나오고, 논문이 알고리즘으로 제시한 세 번째 기여인 계층화된 eviction index는 측정된 기여가 가장 작다. $\rho$와 $k$의 실제 값, 32K 관찰 상한과 1000행 SQL 상한이 어떻게 정해졌는지도 본문에 없다.
