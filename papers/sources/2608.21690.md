...
figures_described: {by: openrouter/openai/gpt-5.6-luna, at: '2026-09-05T15:37:32Z', count: 4, failed: 0}
---
...
![Two side-by-side panels. Left panel titled “Traditional context management” with subtitle “manage the serialized text inside the prompt”. A “Historical sessions” box lists “s-12 prefers economy” and “s-31 avoid toll roads”; an “extract” arrow leads to a “memory store”, and a vertical “retrieve” arrow leads from the store to a “model context” box. A “Current session” box, labelled “U = user · A = agent”, contains U “Plan my Tokyo trip.”, A “search_flights” followed by “→ hundreds of fares”, A “search_routes” followed by “→ airport drive options”, an ellipsis, and U “Book a flight I’d like and plan my drive.”; it leads through scissors labelled “truncate / summarize” into the model context. The model context contains “retrieved memory” and “session summary”, each represented by several horizontal lines, followed by an ellipsis. Right panel titled “Scroll: context management as code” with subtitle “the model writes code to construct its working view”. A dashed “tools” box connects to the Session Environment through the vertical label “tool calls · results bind”. A “model” box sends numbered step 1, “exec” and “model-written code”, into a large “Session Environment” box labelled “persistent · outside the prompt”. Inside are an “Event Log” labelled “append-only”, a “Durable storage” box, and a “Python kernel” labelled “namespace persists”. The Event Log has sections “earlier sessions” and “current session” and lists “seq 0087 U prefers economy”, “seq 0912 U avoid toll roads”, “seq 1103 A search_flights”, and “seq 1893 U book one I’d like”. An arrow labelled “persist” and numbered step 2 leads from the event log to Durable storage, which contains “flights.json” and “routes.json”, the label “Payload storage”, a database icon labelled “History.db”, and “SQLite · FTS5”. An arrow labelled “retrieve · compute” and numbered step 3 leads from the event log to the Python kernel, whose entries are “preference: dict ← s-12, s-31”, “flights: DataFrame ← seq 1103”, and “routes: DataFrame ← seq 1487”; a “load” arrow leads from Durable storage back to the kernel. To the right, outside the Session Environment, is a “model context” box labelled “working view”. It contains “current observation — print” followed by “economy fare ✓” and “no-toll route ✓”, then “earlier sessions” and “current session”, with “Flight results ✓” and “Route ✓”. A numbered step 4 arrow labelled “print” leads into this model context. Below a dashed divider labelled “on evict” is an “Eviction Index” labelled “evicted turns”: “Tier 2” with “seq 1000–1400” and the printed bracketed label “flights”; “Tier 1” with “seq 1487” and the printed bracketed text “routes: done”, followed by “next: book · routes.json”; then an ellipsis and “recover: ms.expand(seq_lo, seq_hi)”. A vertical note beside the index reads “↑ order = coarser”. A dashed arrow labelled “next model call” and numbered step 5 leads from the model context back to the model.](images/2608.21690v1/scroll_system_image.png)
...
![A top-to-bottom data-flow diagram. At the top, a wide box is labelled “Model context” with italic “working view”; its upper-right annotation reads “fixed prefix: system prompt + tool defs”. Three upward arrows from the processing cards below lead to three pills in the working view, labelled “3 rows”, “20 previews”, and “2 events + 2 rows”. Beneath are three side-by-side cards. Card 1 has a numbered circle “1”, the label “exec”, and italic “call tools”. Its code reads `flights = search_flights()`\n`routes = search_routes()`\n`print(flights.head(3))`. The annotation below says “both bind in the kernel · only 3 rows print”. Card 2 has a numbered circle “2”, “exec”, and italic “locate history”. Its code reads `previews = ms.search(`\n`](images/2608.21690v1/scroll_dataflow.png)
...
![Grouped bar chart with vertical axis labelled “mean judge score”, ranging from 0.00 to 1.00 with ticks every 0.25. The legend entries, in left-to-right bar order within each group, are “Lossy summarization”, “Scroll w/o REPL”, “Scroll w/o index”, and “Scroll”. Except for the explicitly annotated Overall group, values are read approximately against the axis. Categories and approximate values in legend order are: “Info extr.” 0.75, 0.75, 0.75, 0.75; “Multi-sess.” 0.20, 0.22, 0.25, 0.22; “Event ord.” 0.20, 0.55, 0.60, 0.64; “Temporal” 0.48, 0.50, 0.49, 0.46; “Knowl. upd.” 0.04, 0.82, 0.93, 0.91; “Contra. res.” 0.04, 0.86, 0.91, 0.89; “Summar.” 0.56, 0.64, 0.70, 0.71; “Pref. foll.” 0.50, 0.80, 0.75, 0.88; “Instr. foll.” 0.22, 0.84, 0.96, 0.96; “Abstention” 0.84, 0.68, 0.84, 0.84; “Overall” is printed on the bars as 0.20, 0.66, 0.71, and 0.73. The leaders are a tie among all methods for Info extr.; Scroll w/o index for Multi-sess. and Event ord.; Scroll w/o REPL, narrowly, for Temporal; Scroll w/o index for Knowl. upd. and Contra. res.; Scroll for Summar., Pref. foll., and the overall score; a near tie between Scroll w/o index and Scroll for Instr. foll.; and a tie among Lossy summarization, Scroll w/o index, and Scroll for Abstention. The largest visible overall gap is between Scroll at 0.73 and Lossy summarization at 0.20.](images/2608.21690v1/beam_ablation.png)
...
![Three-panel box-and-whisker chart with the same four categories, in left-to-right order, LongMemEval-S, BEAM-10M, LOCA-128K, and LOCA-256K. The legend entries are Mean and Median; boxes show the interquartile ranges, horizontal lines inside them show medians, whiskers extend beyond the boxes, and detached points show outliers. Panel (a), `Input tokens`, has a logarithmic vertical axis with labeled ticks at 10^4 and 10^5. Printed mean values are 37.1k, 146.3k, 20.8k, and 20.5k, respectively. Approximate medians read from the axis are 30k, 140k, 20k, and 20k. BEAM-10M has the largest mean and broadest spread, while the two LOCA distributions are compact and have nearly equal means. Panel (b), `Output tokens`, has a logarithmic vertical axis with labeled ticks at 10^2, 10^3, and 10^4. Printed mean values are 1729, 5456, 627, and 598.3, respectively. Approximate medians are 900, 4.5k, 600, and 600; BEAM-10M is highest, LongMemEval-S is next, and the two LOCA distributions are lower and similar. Panel (c), `Interaction turns`, has a vertical axis labeled 0, 50, and 100. Printed mean values are 5.4, 9.1, 35.8, and 39.0, respectively. Approximate medians are 5, 8, 34, and 37; LOCA-256K is highest, closely followed by LOCA-128K, and both have substantially more interaction turns than LongMemEval-S and BEAM-10M. Outlier points are visible above the whiskers in all three panels, including high outliers for the first two categories in panels (a) and (b) and occasional high outliers for the LOCA categories in panel (c).](images/2608.21690v1/cost.png)
